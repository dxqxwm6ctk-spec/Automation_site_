...
CREATE INDEX idx_workflows_active      ON workflows(workspace_id, is_active) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_credentials_type      ON credentials(workspace_id, credential_type);
```
...
CREATE TABLE variables (
  id           uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id uuid NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  workflow_id  uuid REFERENCES workflows(id) ON DELETE CASCADE,  -- null = workspace-level
  ...
  created_at   timestamptz NOT NULL DEFAULT now(),
  updated_at   timestamptz NOT NULL DEFAULT now(),
  UNIQUE (workspace_id, workflow_id, environment, key)
);
...
| Credentials by type | btree on `(workspace_id, credential_type)` | Node config dropdown population |
| Soft-deleted exclusions | partial index `WHERE deleted_at IS NULL` | Avoid dead-row scans |
| Audit log by actor | btree on `(actor_id, created_at DESC)` | User activity timeline |
...
- `audit_logs`: partition by month; retain for 1 year minimum (compliance); never delete
